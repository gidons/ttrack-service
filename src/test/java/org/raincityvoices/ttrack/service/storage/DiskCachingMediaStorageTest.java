...
public class DiskCachingMediaStorageTest {
    ...
    private static final String NO_ETAG = FileMetadata.UNKNOWN.etag();
    ...
    public void setup() throws IOException, UnsupportedAudioFileException {
        ...
        when(fileSystem.getAudioFileFormat(anyFile())).thenReturn(AUDIO_FORMAT);
        when(fileSystem.rename(anyFile(), anyFile())).thenReturn(true);
    }
    ...
    void GIVEN_nothing_in_cache_WHEN_getMedia_THEN_download() throws IOException, UnsupportedAudioFileException {
        ...
        when(fileSystem.getInputStream(anyFile())).thenReturn(mediaStream);
        ...
        verify(fileSystem).exists(mediaFile(ETAG_1));
        // download
        verify(remote).exists(TEST_LOCATION);
        verify(remote).downloadMedia(TEST_LOCATION, FileMetadata.UNKNOWN, downloadFile());
        // local update
        verify(fileSystem).rename(downloadFile(), mediaFile(ETAG_1));
        verify(fileSystem).getInputStream(mediaFile(ETAG_1));
        verify(fileSystem).getAudioFileFormat(argThat(oneOf(mediaFile(ETAG_1), downloadFile())));
    ...
    void GIVEN_file_in_cache_and_etag_is_different_WHEN_getMedia_THEN_update() throws IOException, UnsupportedAudioFileException {
        ...
        when(fileSystem.exists(mediaFile(ETAG_1))).thenReturn(true);
        ...
        when(fileSystem.exists(mediaFile(ETAG_2))).thenReturn(false);
        AudioInputStream mediaStream = newInputStream();
        when(fileSystem.getInputStream(anyFile())).thenReturn(mediaStream);
        ...
        verify(remote).fetchMetadata(TEST_LOCATION);
        verify(fileSystem).exists(mediaFile(ETAG_1));
        verify(fileSystem).getAudioFileFormat(mediaFile(ETAG_1));
        // download
        verify(remote).exists(TEST_LOCATION);
        verify(remote).downloadMedia(TEST_LOCATION, TEST_METADATA, downloadFile());
        // local update
        verify(fileSystem).rename(downloadFile(), mediaFile(ETAG_2));
        verify(fileSystem).getInputStream(mediaFile(ETAG_2));
        verify(fileSystem).getAudioFileFormat(argThat(oneOf(mediaFile(ETAG_2), downloadFile())));
    ...
    void GIVEN_nothing_in_cache_or_remote_WHEN_putMedia_THEN_upload_and_save_in_cache() throws IOException, UnsupportedAudioFileException {
        ...
        verify(fileSystem).writeAudio(mediaInStream, uploadFile());
        verify(fileSystem).getAudioFileFormat(uploadFile());
        ...
        verify(fileSystem).rename(uploadFile(), mediaFile(ETAG_1));
        verify(fileSystem, atMostOnce()).getAudioFileFormat(mediaFile(ETAG_1));
